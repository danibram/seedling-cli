...
import { readJSON, getGitFolder, spawn, parseRemoteLS } from '../helpers'
...
export const info = async function(CWD) {
    ...
    let result = await spawn(null, 'git', 'ls-remote -t', file.repository)
    let parsedResult = parseRemoteLS(result)
